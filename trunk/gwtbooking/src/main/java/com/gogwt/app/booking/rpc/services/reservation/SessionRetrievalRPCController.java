...
public class SessionRetrievalRPCController extends ReservationProcessServiceAdapter {
	 /**
	    ...
	    throws AppRemoteException
	  {
		    ...
		    final ReservationContainerBean sessionReservationContainerBean = 
		    	SessionBeanLookupService.getReservationSessionManager().getReservationContainerBean();
		    
		    final ReservationContainerBean reservationContainerBean = new ReservationContainerBean();
		    
		    checkSessionTimeOut(processStatusEnum, reservationContainerBean);
		    
		    // ProcessStatusEnum.SEARCH_FORM
		    if ( processStatusEnum == ProcessStatusEnum.SEARCH_FORM ) {
		    	reservationContainerBean.setStatus( ProcessStatusEnum.SEARCH_FORM );
		    ...
		    if ( processStatusEnum == ProcessStatusEnum.SEARCH_RESULT ) {
		    	...
		    	reservationContainerBean.setStatus( ProcessStatusEnum.SEARCH_RESULT );
			      reservationContainerBean.setHotelSearchRequest( sessionReservationContainerBean.getHotelSearchRequest() );
			      if (sessionReservationContainerBean.getHotelSearchResponse() != null) 
			      {
			    	  reservationContainerBean.setHotelSearchResponse(sessionReservationContainerBean.getHotelSearchResponse());
			      }
