...
public class WidgetStyleUtils {
	 ...
	 * @return @
	 */
	public static HTML createHtmlLink(final String linkName,
			final String linkUrl) {
		final HTML html = new HTML();
		String formattedlinkName = linkName.replaceAll("[@]", "&#64;");
		// String formattedlinkName = URL.encode(linkName);
		html.setHTML("<a href=\"" + linkUrl + "\" title=\"" + formattedlinkName
				+ "\">" + formattedlinkName + "</a>");
		return html;
	}
	...
	public static Widget addIdStylesToWidget(Widget widget, String id,
			String... styles) {
		widget.getElement().setId(id);
		addStylesToWidget(widget, styles);
		return widget;
	}

	/**
	 * <p>
	 * Use this method when an Anchor widget is needed that makes use of the
	 * href attribute to redirect the user to some external url
	 * </p>
	 * 
	 * @param name
	 * @param href
	...
	public static Label createStyledLabel(String text, boolean wordWrap,
			String... styles) {
		Label label = new Label(text);
	...
	public static VerticalPanel createVerticalPanel(String... styles) {
		final VerticalPanel vp = new VerticalPanel();
		addStylesToWidget(vp, styles);
		return vp;
	}
