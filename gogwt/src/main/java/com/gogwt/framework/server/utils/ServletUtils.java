...
public abstract class ServletUtils {
	 ...
	 * @param pName
	 * @param pValue
	 * @return URL with given parameter (name & value) inserted as first
	...
	public static String addURLParam(final String pURL, final String pName,
		...
		if (pName != null) {
			...
			try {
				...
				newURL.append(java.net.URLEncoder.encode(paramValue,
						DEFAULT_ENCODE));
			} catch (UnsupportedEncodingException e) {
				sLogger.info("Encoding error while trying to encode this name ("
						+ pName + ") or this value: " + paramValue);
			}
	...
	public static String removeParameter(final String pQueryString,
		...
		if (StringUtils.isSet(pQueryString) && StringUtils.isSet(pParameter)) {
			...
			if (startPos != -1) {
				...
				// for whatever reason, catch this exception so we don't kill
				// the
				// request
	...
	public static boolean absoluteURL(String url) {
		return StringUtils.containsString(url, HTTPS)
				|| StringUtils.containsString(url, HTTP);
	}
	 ...
	 *            - the request.
	 */
	public static String debugRequestParameters(
	 ...
	 * @param value
	 */
	private static void addNameValuePair(StringBuffer sBuf, String name,
			String value) {
